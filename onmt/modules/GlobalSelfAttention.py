...
class GlobalSelfAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    """
    def __init__(self, dim, coverage=False, attn_type="dot", attn_hidden=0, no_gate=False, residual=False, no_gate_relu=False, no_gate_bias=False):
        super(GlobalSelfAttention, self).__init__()

        self.no_gate = no_gate
        self.residual = residual

        print("no gate status is {} and residual status is {}, relu {} gate bias {}\n".format(self.no_gate, self.residual, no_gate_relu, no_gate_bias))
        ...
        if self.no_gate:
            self.linear_out = nn.Sequential(nn.Linear(dim*2, dim, bias=no_gate_bias), 
                nn.ReLU() if no_gate_relu else nn.Tanh())
        else:
